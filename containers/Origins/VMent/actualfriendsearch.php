...
if (isset($_POST["search_term"])) {
	...
	$search = mysql_query("SELECT * FROM `friends` WHERE `user_id` = $session_user_id OR `friend_id` = $session_user_id AND `friend_username` LIKE '%$search_term%' AND `active` = 1 OR `user_name` LIKE '%$search_term%' AND `active` = 1");	
