...
<?php 
$check_query = mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 1");
if (mysql_num_rows($check_query) != 0) {
$friend_check = mysql_result($check_query, 0);
}
...
if (empty($_POST) === false) {
	...
	$check_request_query = mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 0");
	if (mysql_num_rows($check_request_query) != 0) {
		$check_request = mysql_result($check_request_query, 0);
		} else {
		$check_request = 0;
		}
	...
	//echo "";
		...
		?>		
		<button class="buttons" value="Add as VFriend!" onClick="self.location='friendrequest.php?request=<?php echo $profile_data["user_id"]; echo '&friend_username=' . $profile_data["username"];
...
while (post_count($profile_data["user_id"]) > $n) {
	...
	$query = mysql_query("SELECT `post_id` FROM `posts` WHERE $user_id = `to` LIMIT $n, 100");
  ...
  if ($num_rows > 0) {
	...
	$post_data = post_data($post_id, "from", "to", "content", "content_type", "no", "cool", "date", "vments");
	$from_data = user_data($post_data["from"], "username", "first_name");
	?>
