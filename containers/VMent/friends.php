...
if (friend_requests_count($session_user_id) > 0 || friend_count($session_user_id) > 0) {
	$check_query = mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 0");
	$num_rows = mysql_num_rows($check_query) or die(mysql_error());
	if ($num_rows != 0) {
  $check_user = mysql_result($check_query, 0);
...
while (friend_count($session_user_id) > $n) {
	$query = mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 1 AND $session_user_id != `user_id` AND `friend_id` = $session_user_id LIMIT $n, 100");
	$num_rows = mysql_num_rows($query);
  if ($num_rows > 0) {
	 $friend_id = mysql_result($query, 0);
	 } else {
	 $query = mysql_query("SELECT `friend_id` FROM `friends` WHERE `active` = 1 AND `user_id` = $session_user_id");
	 $num_rows = mysql_num_rows($query);
	 if ($num_rows > $n) {
...
$n++;
  ...
  <?php
	if (friend_requests_count($session_user_id) !== false && friend_requests_count($session_user_id) > 0 ) {
		$friend_id = mysql_result(mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 0"), 0);
	...
	$friend_id = mysql_result(mysql_query("SELECT `user_id` FROM `friends` WHERE `active` = 0"), 1);
	$friend_data = user_data($friend_id, "user_id", "username", "first_name", "last_name", "profile");
	?>
	<div>Profile:<br><a href="<?php echo $friend_data["username"]; ?>">
		<img style="border-radius: 15px;" id="profilepic" heigth="10%" width="10%" src="<?php echo $friend_data["profile"] ?>" alt="<?php echo $friend_data["username"] ?>'s profile pic">
