...
{
	global $connect;
	return mysqli_num_rows(mysqli_query($connect, "SELECT `post_id` FROM `posts` WHERE `to` = $user_id"));
}
...
function make_post_apply($connect, $post_data)
{
...
{
	function exist_check($connect, $friend_id, $user_id)
	{
...
function friend_accept($connect, $friend_id, $user_id)
{
	mysqli_query($connect, "UPDATE `friends` SET `active` = 1 WHERE `user_id` = $user_id AND `friend_id` = $friend_id");
}
function friend_ignore($connect, $friend_id, $user_id)
{
	mysqli_query($connect, "DELETE FROM `friends` WHERE `user_id` = $user_id AND `friend_id` = $friend_id");
}
function unfriend($connect, $friend_id, $user_id)
{
	mysqli_query($connect, "DELETE FROM `friends` WHERE `user_id` = $user_id AND `friend_id` = $friend_id OR `user_id` = $friend_id AND `friend_id` = $user_id");
}
function friend_request_apply($connect, $request_data)
{
...
{
	global $connect;
	if (!mysqli_num_rows(mysqli_query($connect, "SELECT `id` FROM `friends` WHERE `active` = 0 AND `friend_id` = $friend_id"))) {
		return 0;
	} else {
...
function user_id_from_username($username)
{
	global $connect;
	$username = sanitize($username);
	return mysqli_data_seek(mysqli_query($connect, "SELECT `user_id` FROM `users` WHERE `username` = '$username'"), 0, "user_id");
}
