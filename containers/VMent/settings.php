...
<?php
		...
		while (block_count($session_user_id) > $n) {
			$query = mysqli_query($connect, "SELECT `user_id` FROM `friends` WHERE `active` = 2 AND $session_user_id != `user_id` AND `friend_id` = $session_user_id LIMIT $n, 100");
			$num_rows = mysqli_num_rows($query);
			$beforeid = $friend_data["user_id"];
			if ($num_rows > 0) {
				$friend_id = mysqli_data_seek($query, 0);
			} else {
				$query = mysqli_query($connect, "SELECT `friend_id` FROM `friends` WHERE `active` = 2 AND `user_id` = $session_user_id");
				$num_rows = mysqli_num_rows($query);
				if ($num_rows > $n) {
