...
if (isset($_POST["search_term"])) {
	...
	$search = mysqli_query($connect, "SELECT * FROM `users` WHERE `first_name` LIKE '%$search_term%' AND `active` = 1 OR `last_name` LIKE '%$search_term%' AND `active` = 1 OR `username` LIKE '%$search_term%' AND `active` = 1");	
