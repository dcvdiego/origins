...
while (post_count($session_user_id) > $n) {
	$query = mysql_query("SELECT `post_id` FROM `posts` WHERE $session_user_id = `to` ORDER BY  `posts`.`post_id` DESC LIMIT $n, 100");
  ...
  if ($num_rows > 0) {
	...
	if ($date === date("Y-m-d")) {
		...
		<br>
		<br>
		<a href="?cool=<?php echo $post_id; ?>"><div class="buttons"  name="cool"><?php echo $post_data["cool"]; ?> Cool</div></a>
		<a href="?no=<?php echo $post_id;?>"><div class="buttons" name="no"><?php echo $post_data["no"];
	$check = mysql_result(mysql_query("SELECT `type` FROM `poststuff` WHERE `user_id` = $session_user_id AND `post_id` = $post_id"), 0);
	//doesnt work
	if ($check === "") {
	echo " No<br></div></a>";	
	} else {
		echo " Unno<br></div></a>";
			  ...
			  ?>
		<a href="?vment=<?php echo $post_id; ?>"><div class="buttons" name="vment"><?php echo $post_data["vments"];?> VMENTS<br></div></a>
</div>
	<br>
	<br>
<?php
